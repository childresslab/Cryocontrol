...
if __name__ == "__main__":
    ...
    fig,ax = plt.subplots()
    ax.errorbar(cut_times,cut_counts,np.sqrt(cut_counts),fmt='o',linestyle=None,ms=2,alpha=0.5,zorder=3,label="Data")
    ...
    func_eval = single_gexp(new_cut_times,
                            ...
                            pparams['lifetime'],
                            pparams['offset'])
    ax.plot(new_cut_times,func_eval,zorder=6,label="Fit")
    ax.set_yscale('log')
    ax.legend(loc="upper right",
              title=f"tau = {pparams['lifetime']:.2f}±{decay.params['lifetime'].stderr:.2f} ns\nchi2red = {decay.redchi:.3f}±{np.sqrt(2/decay.nfree):.1e}")
