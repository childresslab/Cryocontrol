...
from fpga_cryo_dummy import DummyCryoFPGA
...
def save_counts(*args):
    ...
    with path.open('w') as f:
        f.write("Timestamp,Counts\n")
        for i, time in enumerate(counts_data['time']):
            f.write(f"{time},{counts_data['counts'][i]}\n")
...
def set_galvo_cav_count(x,y,z):
    set_galvo(x,y,write=False)
    set_cavity(z)
    count = fpga.just_count(dpg.get_value("Cavity Scan/Count Time (ms)"))
    log.debug(f"Got count rate of {count}.")
    if dpg.get_value("Counts/Plot Scan Counts"):
        counts_data['counts'].append(count)
        counts_data['time'].append(datetime.now().timestamp())
...
with dpg.window(label="Confocal Scanner Window", tag='main_window'):
    ...
    with dpg.group(horizontal=True):
        ...
        dpg.add_button(tag="optimize", label="Optimize Galvo TODO", callback=optimize_galvo)
            ...
            with dpg.group(horizontal=True,width=0):
                ...
                with dpg.child_window(width=-1,autosize_y=True): 
                    ...
                    with dpg.plot(label="Count Rate",width=-1,height=-1,tag="count_plot"):
                        ...
                        dpg.add_plot_axis(dpg.mvXAxis, label="x", time=True, tag="count_x")
                        dpg.add_plot_axis(dpg.mvYAxis, label="y",tag="count_y")
